...
public class waterSkil : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha3)) // Замените на нужную клавишу
        {
    ...
    void ChangeSoilTilesInArea(Vector3Int centerPosition, Vector2Int offset)
    {
        for (int x = 0; x < 2; x++)
        {
            for (int y = 0; y < 2; y++)
            {
                ...
                tilemap.SetTile(position, waterTile); // Устанавливаем новый тайл, выбранный в инспекторе
