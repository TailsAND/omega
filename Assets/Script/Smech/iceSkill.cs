...
public class IceSkil : MonoBehaviour
{
    ...
    public TileBase iceTile; // Тайл льда
        ...
        for (int x = 0; x < 2; x++)
        {
            for (int y = 0; y < 2; y++)
            {
                ...
                if (tilemap.HasTile(position) && tilemap.GetTile(position) is TileBase)
                {
                    ...
                    if (originalTile != fireTile && originalTile != waterTile && originalTile != iceTile)
                    {
                ...
                tilemap.SetTile(position, iceTile); // Устанавливаем новый тайл, выбранный в инспекторе
    ...
    public void CheckForWaterCreation(Vector3Int position)
    {
        ...
        if (hasIce && hasFire)
        {
            tilemap.SetTile(position, waterTile); // Устанавливаем тайл воды
        }
        else
        {
